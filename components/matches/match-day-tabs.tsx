    ...
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-2 rounded-lg border bg-card p-1">
        {days.map((day) => (
          <button
            className={cn(
              "h-10 rounded-lg text-sm font-medium text-muted transition-colors hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent",
              activeDay === day && "bg-accent text-black hover:text-black"
            )}
            key={day}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-xl font-semibold text-text">{activeDay}</h2>
