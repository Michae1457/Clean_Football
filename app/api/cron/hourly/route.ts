...
export async function GET(request: NextRequest) {
  ...
  const maxPerSource =
    readPositiveInteger(
      process.env.CRON_HOURLY_MAX_PER_SOURCE ??
        process.env.CRON_MAX_PER_SOURCE ??
        process.env.NEWS_REFRESH_LIMIT_PER_SOURCE ??
        process.env.NEWS_FETCH_LIMIT_PER_SOURCE
    ) ?? 6;
  const useNewsAi = readBooleanFlag(
    process.env.CRON_HOURLY_NEWS_USE_AI ??
      process.env.CRON_NEWS_USE_AI ??
      process.env.AI_ENABLE_ARTICLE_SUMMARY,
    false
  );
  const payload: HourlyCronPayload = {
    date,
    errors: [],
    jobs: {},
    ok: true,
